...
async function updateDockerServices(docker) {
  ...
  for (let { service, existingService } of servicesData) {
    const [imageName, currentDigest] = existingService.Spec.TaskTemplate.ContainerSpec.Image.split('@');
    ...
    if (image.RepoDigests.length === 0) {
      console.log('No RepoDigests for for', imageName);
      continue;
    }

    const latestImageDigest = image.RepoDigests[0].split('@sha256:')[1];
    console.log(imageName);
    console.log('latestImageDigest:', latestImageDigest);
    console.log('currentDigest', currentDigest);

    if (currentDigest?.find(imageId => imageId !== latestImageDigest)) {
